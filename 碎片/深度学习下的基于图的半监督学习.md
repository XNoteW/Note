...
其中, $1\leq i\leq l$, $\alpha^{(i)}$ 有标签 $y^{(i)}$, 对于分类任务, $y^{(i)} \in \{1, 2, \cdots, C\}$ 或者 $y^{(i)} \in {\{0,1\}}^C$ (当 $y^{(i)}$ 为类别 $k \in \{1, 2, \cdots, C\}$ 时, $y^{(i)}$ 的第 $k$ 个元素是 $1$, 其余元素均为 $0$, 即 One-hot[^1] 编码的形式), 其中 $C$ 为类别数目.
